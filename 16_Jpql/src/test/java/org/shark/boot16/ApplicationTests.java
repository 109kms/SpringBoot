...
class ApplicationTests {
  ...
  void distinctTest() {
    ...
    TypedQuery<String> typedQuery = em.createQuery(jpql, String.class);
    List<String> logLevels = typedQuery.getResultList();
    logLevels.forEach(logLevel -> log.info("logLevel: {}", logLevel));
  }
  // 2. logId가 1에서 5인 엔티티를 조회하세요.
  @Test
  @DisplayName("BETWEEN AND 연산자 테스트")
  void betweenAndOperatorTest() {
    String jpql = "SELECT al FROM AccessLog al WHERE al.logId BETWEEN :startId AND :endId";
    TypedQuery<AccessLog> typedQuery = em.createQuery(jpql, AccessLog.class)
                                         .setParameter("startId", 1)
                                         .setParameter("endId", 5);
    List<AccessLog> accessLogs = typedQuery.getResultList();
    accessLogs.forEach(accessLog -> log.info("{}", accessLog));
  }
  // 3. logId가 1 또는 5인 엔티티를 조회하세요.
  @Test
  @DisplayName("IN 연산자 테스트")
  void inOperatorTest() {
    String jpql = "SELECT al FROM AccessLog al WHERE al.logId IN (:id1, :id2)";
    TypedQuery<AccessLog> typedQuery = em.createQuery(jpql, AccessLog.class)
                                         .setParameter("id1", 1)
                                         .setParameter("id2", 5);
    List<AccessLog> accessLogs = typedQuery.getResultList();
  ...
  void likeOperatorTest() {
    String jpql = "SELECT al FROM AccessLog al WHERE al.logType LIKE :level";
    TypedQuery<AccessLog> typedQuery = em.createQuery(jpql, AccessLog.class)
                                         .setParameter("level", "%USER%");
    List<AccessLog> accessLogs = typedQuery.getResultList();
    accessLogs.forEach(accessLog -> log.info("{}", accessLog));
  }
